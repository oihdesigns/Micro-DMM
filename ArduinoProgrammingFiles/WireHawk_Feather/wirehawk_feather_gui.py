#!/usr/bin/env python3
"""
wirehawk_feather_gui.py  --  Host GUI for WireHawk_Feather (Feather RP2350).

Same protocol as the original WireHawk GUI, plus the open-lead detector that
the Feather build folds in: the board reports a lead code (NONE / FLOAT /
CLOSED / VOLTAGE), the lead voltage, and the bridge average alongside every
current sample.

Talks to the board over USB serial (115200 baud).  Four tabs:

  Test
    - MOTOR ON / MOTOR OFF side by side.  ON needs no ceremony when the board
      can SEE a motor (leads CLOSED / state READY); with nothing detected it is
      disabled until the Override box is ticked, because that is the case where
      closing the relay is the questionable act.  OFF is never gated and never
      disabled, and Esc stops from anywhere in the window
    - big live current readout, run timer, and a large diagnosis banner sitting
      directly above the plot
    - avg / min / max / peak / ripple statistics
    - lead-detector panel: open/closed verdict, estimated resistance, and a
      large volts readout whenever the board reports voltage on the leads.
      This replaced the load-side sense-bit display -- the board still drives,
      streams and logs the D11/D10 bits (and USEIO still works), but the lead
      detector answers the same question directly, so it is not shown
...
Run:
    python wirehawk_feather_gui.py
"""
...
# Must match STATE_NAMES / State in WireHawk_Feather.ino.
STATES = {
...
# Lead code from the open-lead detector -> (label, bg, fg, explanation).
# The value is also what the board drives on its two lead-status pins.
LEADS = {
    0: ("NO TEST", "#303030", "#c8c8c8",
        "no lead result: test suppressed (!CFSUP), no lead ADC, or the relay "
        "is closed so the bridge test is interlocked out"),
    1: ("OPEN",    "#5a3a12", "#ffcc7e",
        "01  leads are open -- no continuity between them"),
    2: ("CLOSED",  "#0d5a1e", "#7dff8f",
        "10  leads are joined -- continuity through the load"),
    3: ("VOLTAGE", "#6a5a0d", "#ffe97e",
        "11  voltage measured on the leads -- no lead test possible"),
}
...
KEY_META = {
    ...
    # --- Diagnosis ---
    "USEIO":     ("Diagnosis", "bool", "1 = use the D11/D10 sense bits (0 = current only)"),
    "IOINVERT":  ("Diagnosis", "bool", "1 = sense bits are active low"),
    "IOMODE":    ("Diagnosis", "choice", "D11/D10 input mode: 0=pulldown 1=pullup 2=plain",
                  ["0", "1", "2"]),
    ...
    # --- Lead detector ---
    "LEADEN":    ("Lead detector", "bool", "1 = run the open-lead subsystem"),
    "LEADHOT":   ("Lead detector", "bool",
                  "1 = allow the bridge test with the relay CLOSED. Leave 0 "
                  "unless the load is isolated -- the bridge would otherwise be "
                  "switched across live leads"),
    "LEADMS":    ("Lead detector", "num",  "Lead measurement interval (ms)"),
    "ACMODE":    ("Lead detector", "bool", "1 = AC/VRMS lead measurement"),
    "CORFTRIG":  ("Lead detector", "num",
                  "|V| below this counts as dead leads, so the bridge test may run"),
    "CLSDTHRS":  ("Lead detector", "num",
                  "Bridge volts below this = CLOSED, above = OPEN"),
    "BRBASEV":   ("Lead detector", "num",
                  "Bridge volts at the ~10K calibration point (0% confidence)"),
    "VSCALE":    ("Lead detector", "num",
                  "Counts-to-volts scale with the bridge disconnected"),
}
GROUP_ORDER = ["Current sense", "Diagnosis", "Safety", "Lead detector",
               "Capture / reporting", "Other"]
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("WireHawk Feather -- Motor Test, Fault Diagnosis & Lead Detect")
        ...
        self.relay = 0
        self.io_code = 0
        self.lead_code = None          # None until the board reports one
        self.lead_extra = ""           # resistance estimate from the last $LEAD
    ...
    def _build_ui(self):
        ...
        # The diagnosis banner lives above the plot on the Test tab (built in
        # _build_test_tab), not here -- so it is big enough to read across the
        # bench rather than being a strip over the whole window.
    ...
    def _build_test_tab(self):
        ...
        # --- open-lead detector ---
        lead = ttk.LabelFrame(left, text="Leads (open / closed)", padding=8)
        lead.pack(fill="x", pady=6)
        self.lead_lbl = tk.Label(lead, text="--", bg="#303030", fg="#c8c8c8",
                                 font=("Segoe UI", 13, "bold"), padx=8, pady=3)
        self.lead_lbl.pack(fill="x")
        # Big volts readout, shown only while the board actually reports voltage
        # on the leads -- an idle number here would be read as a live one.
        self.lead_volts = tk.Label(lead, text="", bg="#6a5a0d", fg="#ffe97e",
                                   font=("Consolas", 24, "bold"))
        self.lead_detail = ttk.Label(lead, text="waiting for the board",
                                     wraplength=210, justify="left")
        self.lead_detail.pack(pady=(6, 0))
        self.lead_num = ttk.Label(lead, text="", font=("Consolas", 9))
        self.lead_num.pack(anchor="w", pady=(4, 0))
        ttk.Button(lead, text="Suppress lead test (toggle)",
                   command=lambda: self.send("!CFSUP")).pack(fill="x", pady=(6, 0))

        # The load-side sense bits (D11/D10) have no panel any more: the lead
        # detector reports open/closed/voltage directly, so the two-bit code is
        # a redundant second opinion on screen.  The board still drives and
        # streams it -- the $WH d10/d11/io fields and the CSV columns are
        # untouched, and USEIO still works -- it is just not displayed.

        # --- relay control ---
        ctl = ttk.LabelFrame(left, text="Relay / motor", padding=8)
        ctl.pack(fill="x", pady=6)
        # A detected motor is its own permission to energise; Override is only
        # needed when the board CANNOT see a load, which is when closing the
        # relay is the questionable act (open leads, or lead detection off).
        self.override = tk.BooleanVar(value=False)
        ttk.Checkbutton(ctl, text="Override (energise with no motor detected)",
                        variable=self.override,
                        command=self._update_arm).pack(anchor="w")
        self.arm_lbl = ttk.Label(ctl, text="", foreground="#a06000",
                                 wraplength=210, justify="left")
        self.arm_lbl.pack(anchor="w", pady=(2, 0))
        # ON and OFF side by side.  OFF is never disabled and never gated by the
        # Arm checkbox -- de-energising must always be one click away, whatever
        # state the GUI thinks the board is in.
        onoff = ttk.Frame(ctl)
        onoff.pack(fill="x", pady=(6, 2))
        onoff.columnconfigure(0, weight=1, uniform="onoff")
        onoff.columnconfigure(1, weight=1, uniform="onoff")
        self.on_btn = tk.Button(onoff, text="MOTOR ON", bg="#166a22", fg="white",
                                font=("Segoe UI", 11, "bold"), state="disabled",
                                activebackground="#1e8c2e", command=self.cmd_on)
        self.on_btn.grid(row=0, column=0, sticky="ew", padx=(0, 3))
        self.off_btn = tk.Button(onoff, text="MOTOR OFF", bg="#8a1010", fg="white",
                                 font=("Segoe UI", 11, "bold"),
                                 activebackground="#b02020", command=self.cmd_off)
        self.off_btn.grid(row=0, column=1, sticky="ew", padx=(3, 0))
        ...
        # --- diagnosis banner, directly above the plot ---
        # Sized to be read from across the bench: this is the one widget that
        # has to be legible while your hands are on the motor, not the mouse.
        banner = tk.Frame(right, bg="#303030")
        banner.pack(fill="x", padx=4, pady=(0, 4))
        self.state_lbl = tk.Label(banner, text="IDLE", bg="#303030", fg="#c8c8c8",
                                  font=("Segoe UI", 40, "bold"), pady=14)
        self.state_lbl.pack(side="left", padx=18)
        self.detail_lbl = tk.Label(banner, text="not connected", bg="#303030",
                                   fg="#909090", font=("Segoe UI", 13),
                                   justify="left", anchor="w", wraplength=430)
        self.detail_lbl.pack(side="left", fill="x", expand=True)
    ...
    def _reset_history(self):
        ...
        self.hist_rows = []
        self.relay = 0
        self._clear_lead("board reset -- waiting for a lead report")
    ...
    def _motor_present(self):
        """Is a load actually attached?

        The lead detector answers this directly -- CLOSED means continuity
        between the leads -- and ST_READY is the firmware's own version of the
        same finding.  Anything else (OPEN leads, VOLTAGE, NO TEST, lead
        detection disabled, no lead ADC) is NOT a detection, so it falls
        through to Override rather than being treated as a maybe.
        """
        return self.lead_code == 2 or self.state_code == 1

    def cmd_allowed(self):
        """May we energise?  A detected motor, or an explicit Override.

        `relay == 1` also passes: the motor is already running, so the question
        is moot, and greying the buttons out mid-run would read as the GUI
        having lost control of a live motor.
        """
        return self.serial.is_open and (self._motor_present()
                                        or self.relay == 1
                                        or self.override.get())

    def _update_arm(self):
        ok = self.cmd_allowed()
        # Change-guarded: this is driven off the telemetry stream, so it runs
        # at the $WH rate.
        if ok != getattr(self, "_arm_state", None):
            self._arm_state = ok
            state = "normal" if ok else "disabled"
            self.on_btn.config(state=state)
            self.run_btn.config(state=state)

        present = self._motor_present()
        if not self.serial.is_open:
            hint, colour = "", "#a06000"
        elif self.relay == 1:
            hint, colour = "relay CLOSED -- motor energised", "#a04000"
        elif present:
            hint, colour = "motor detected -- ready to energise", "#308030"
        elif self.override.get():
            hint, colour = "OVERRIDE: no motor detected, energising anyway", "#a06000"
        else:
            hint, colour = "no motor detected -- tick Override to energise", "#a06000"
        if hint != getattr(self, "_arm_hint", None):
            self._arm_hint = hint
            self.arm_lbl.config(text=hint, foreground=colour)
    ...
    def cmd_capture(self):
        try:
            ...
            return
        if not self.cmd_allowed() and not messagebox.askyesno(
                "WireHawk", "A capture CLOSES THE RELAY and energises the motor for "
                            "%d ms.\n\nNo motor is detected and Override is off.\n\n"
                            "Continue?" % ms):
            return
        self.cap_rows = []
        self.cap_header = None
    ...
    def _seq_step2(self):
        ...
        self.seq_idle_lead = self.lead_code
        self._append_verdict("Idle: %s, leads %s\n" % (
            STATES.get(self.seq_idle_state, ("?",))[0],
            LEADS.get(self.seq_idle_lead, ("not reported",))[0]))
        try:
    ...
    def _seq_verdict(self):
        ...
        if self.seq_idle_state == 2:
            ...
            # The capture rows carry the sense bits, not the lead code, so this
            # points at the live Leads panel rather than guessing from data the
            # capture did not record.
            self._append_verdict(
                "VERDICT: no current with the relay closed. Energise again and watch "
                "the Leads panel: VOLTAGE there means the supply did reach the leads "
                "and the motor is stalled; no voltage means the load never got power "
                "(relay contacts, fuse, or supply).")
    ...
    def toggle_log(self):
        ...
        self.log_writer.writerow(["ms", "relay", "mA", "avgMA", "minMA", "maxMA",
                                  "peakMA", "rippleMA", "mV", "d10", "d11", "io",
                                  "state", "stateName", "runMs",
                                  "seq", "maxLoopUs", "txSkips",
                                  "leadCode", "leadName", "leadV", "bridgeAvg",
                                  "src"])
    ...
    def _sync_from_config(self):
        ...
        if self.host_to_ms == 0:
            self._log("!! HOSTTO=0: the board will NOT drop the relay by itself if "
                      ...
                      "Configuration tab.")
        try:
            lead_en = int(float(self.cfg_rows["LEADEN"]["value"]))
        except (KeyError, ValueError):
            lead_en = None
        if lead_en == 0:
            self._log("!! LEADEN=0: the lead detector is off, so the Leads panel "
                      "will stay blank and the diagnosis runs on current alone. "
                      "Set LEADEN=1 on the Configuration tab.")
    ...
    def _handle_line(self, line, cfg_batch):
        ...
        if tag == "$CAPSTART":
            ...
            # $LEAD,code,name,leadV,bridgeV,bridgeAvg,rEst,conf,vzero,ac,sup
            try:
                code = int(parts[1])
                lead_v = float(parts[3])
                bridge_avg = float(parts[5])
            except (IndexError, ValueError):
                return
            extra = ""
            if len(parts) > 7:
                extra = "   %s (%s%%)" % (parts[6], parts[7])
            self._set_lead(code, lead_v, bridge_avg, extra)
        elif tag == "$CFG":
            if len(parts) >= 3:
            ...
            self._reset_history()
            self.override.set(False)   # a reset must not leave the guard lifted
            self._update_arm()
    ...
    def _on_stream(self, p):
        ...
        # Lead detector fields, appended by the Feather build (older firmware
        # simply stops at txSkips and this block is skipped).
        if len(p) >= 23:
            try:
                self._set_lead(int(p[19]), float(p[21]), float(p[22]))
            except ValueError:
                pass

        if self.log_writer:
            row = list(p[1:23])
            self.log_writer.writerow(row + [""] * (22 - len(row)) + ["live"])
        ...
        self.io_code = io      # still tracked and logged, just not displayed
        ...
        self.relay = relay
        # Permission to energise is derived from this row: lead code, motor
        # state and relay have all just been updated.
        self._update_arm()
    ...
    def _paint_link_lost(self, was_energised, why):
        ...
        self.state_code = -1
        self._clear_lead("link lost -- lead state unknown")
    ...
    def _clear_lead(self, why):
        """Drop the lead readout when the board can no longer vouch for it.

        The volts figure is the one thing here that must never go stale: left on
        screen after a link loss it reads as a live measurement of leads that
        may since have gone dead (or live).
        """
        self.lead_code = None       # forces a repaint from the next report
        self.lead_extra = ""
        self.lead_volts.pack_forget()
        self.lead_lbl.config(text="--", bg="#303030", fg="#c8c8c8")
        self.lead_detail.config(text=why)
        self.lead_num.config(text="")
        self._update_arm()      # detection just went away; re-gate MOTOR ON

    def _set_lead(self, code, lead_v, bridge_avg, extra=None):
        """Paint the lead-detector panel.

        Fed both by the lead fields on every $WH row and by the richer $LEAD
        line the board sends on each change; the $LEAD extras (resistance bin,
        confidence) are sticky so the stream rows do not wipe them.
        """
        if extra is not None:
            self.lead_extra = extra
        label, bg, fg, why = LEADS.get(code, ("?", "#303030", "#c8c8c8", ""))
        live_v = (code == 3)          # VOLTAGE: the reading is a real measurement

        # Text before pack(), so showing the label can never flash the previous
        # reading -- on these leads that is a safety-relevant number.
        if live_v:
            self.lead_volts.config(text="%+.2f V" % lead_v)
        if code != self.lead_code:
            self.lead_code = code
            self.lead_lbl.config(text=label, bg=bg, fg=fg)
            self.lead_detail.config(text=why)
            # Pack/forget rather than blanking, so the panel does not keep an
            # empty voltage row once the leads go dead.
            if live_v:
                self.lead_volts.pack(fill="x", pady=(2, 0), after=self.lead_lbl)
            else:
                self.lead_volts.pack_forget()
        # The resistance estimate only means anything for an actual bridge
        # result; carrying it into VOLTAGE/NO TEST would date-stamp a number
        # the board is no longer measuring.
        extra_txt = self.lead_extra if code in (1, 2) else ""
        self.lead_num.config(text="lead %.2f V   bridge %.3f V%s"
                             % (lead_v, bridge_avg, extra_txt))
        self._update_arm()      # a CLOSED reading is what permits MOTOR ON
