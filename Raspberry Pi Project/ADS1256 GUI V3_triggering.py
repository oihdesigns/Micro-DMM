...
# Trigger controls
tk.Label(logger_frame, text="Low Trigger (V):").grid(row=2, column=0, sticky="w", padx=5)
low_trigger_var = tk.StringVar(value="5.0")
tk.Entry(logger_frame, textvariable=low_trigger_var, width=10).grid(row=2, column=1, padx=5)

tk.Label(logger_frame, text="High Trigger (V):").grid(row=2, column=2, sticky="w", padx=5)
high_trigger_var = tk.StringVar(value="2.5")
tk.Entry(logger_frame, textvariable=high_trigger_var, width=10).grid(row=2, column=3, padx=5)

auto_rearm_var = tk.BooleanVar(value=False)
tk.Checkbutton(logger_frame, text="Auto Re-Arm", variable=auto_rearm_var).grid(row=2, column=5, padx=5)

log_fig, log_ax = plt.subplots(figsize=(5, 2.5), dpi=100)
log_canvas = FigureCanvasTkAgg(log_fig, master=logger_frame)
log_canvas.get_tk_widget().grid(row=1, column=0, columnspan=6, padx=2, pady=2)

stats_frame = tk.Frame(logger_frame)
stats_frame.grid(row=1, column=6, padx=10, sticky="n")
...
def trigger_capture():
    ...
    pre_points = (n_points - 1) // 2
    post_points = n_points - pre_points - 1

    low_thr = float(low_trigger_var.get())
    high_thr = float(high_trigger_var.get())
    ...
    pre_buffer = deque(maxlen=pre_points)
    pre_timestamps = deque(maxlen=pre_points)
    ...
    while True:
        ...
        val = raw * scale
        ts = (time.time() - t0) * 1000.0

        if not triggered:
            pre_buffer.append(val)
            pre_timestamps.append(ts)

            # OR trigger: below low OR above high
            if val < low_thr or val > high_thr:
                triggered = True
                captured.extend(pre_buffer)
                captured.append(val)
                time_stamps = list(pre_timestamps) + [ts]
        else:
            captured.append(val)
    ...
    captured_data = {ch_index: captured}
    timestamps = time_stamps

    log_ax.clear()
    log_ax.plot(timestamps, captured_data[ch_index], label="Trigger Capture")
    log_ax.legend(loc="upper right")
    log_canvas.draw()

    for w in stats_frame.winfo_children(): w.destroy()
    txt = (f"{CHANNEL_MAP[ch_index][0]} (Triggered):\n"
           f"Samples: {n_points}\n"
           f"Min: {min(captured):.6f}\n"
           f"Max: {max(captured):.6f}\n"
           f"Avg: {statistics.mean(captured):.6f}\n"
           f"Std: {statistics.pstdev(captured):.6f}\n")
...
tk.Button(logger_frame, text="Arm Trigger", command=trigger_capture).grid(row=2, column=4, padx=5, pady=5)
